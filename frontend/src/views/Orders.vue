<template>
    <div class="container-fluid mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>Orders</h1>
            <RouterLink to="/new-order" class="btn btn-primary" :class="{ 'disabled': !canCreateOrder }" :aria-disabled="!canCreateOrder">
                Create New Order
            </RouterLink>
        </div>
        ...
        <div v-if="!canCreateOrder && !isLoading" class="alert alert-warning" role="alert">
            ...
            <p>To create a new order, you must first have at least one of each of the following resources. Please use the links below to create them.</p>
            <hr>
        ...
        <div v-else-if="filteredOrders.length > 0" class="row g-4">
            <div v-for="order in filteredOrders" :key="order.id" class="col-lg-6">
                <div class="card h-100 shadow-sm order-card">
                    ...
                    :style="{ backgroundColor: order.status.name === 'Success' ? '#d3ff6e' : order.status.name === 'Failed' ? '#ff7070' : '#affbff' }">
                        ...
                        <span :class="getStatusClass(order.status.name)" :style="{ backgroundColor: order.status.name === 'Success' ? 'green' : 'red' }">{{ order.status.name }}</span>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6">
                    ...
                    <div class="card-footer text-end">
                        <button class="btn btn-sm btn-outline-secondary me-2" @click="viewDetails(order.id)">View Details</button>
                        <button v-if="order.status.name !== 'Success' && order.status.name !== 'Failed'" class="btn btn-sm btn-outline-danger" @click="showDeleteModal(order)">Delete</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete <strong>Order #{{ orderToDelete?.id }}</strong> for client <strong>{{ orderToDelete?.client.name }}</strong>?</p>
                </div>
...
const getPrerequisites = async () => {
    const endpoints = {
        ...
        orderStatuses: '/api/order-status'
    };
    const keys = Object.keys(endpoints)
    const urls = Object.values(endpoints)
    const results = await fetchBatchData(urls)
    results.forEach((data, index) => {
        const key = keys[index]
        prerequisites[key] = data || [];
    });
};
...
const filteredOrders = computed(() => {
    return orders.value.filter(order => {
        ...
        return statusName !== 'success' && statusName !== 'failed';
    });
});

onMounted(async () => {
    isLoading.value = true;
    await Promise.allSettled([getPrerequisites(), getOrders()]);
...
.order-card p { margin-bottom: 0.5rem; }
.card-footer {
