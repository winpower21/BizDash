...
class Partner(db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint('revenue_share >= 0 AND revenue_share <= 1',
                        name='check_revenue_share_range'),
    )
    ...
    settlements = db.relationship(
        'Settlement', back_populates='partner', lazy=True)
...
order_type_documents = db.Table('order_type_documents',
                                db.Column('order_type_id', db.Integer, db.ForeignKey(
                                    'order_type.id', ondelete="CASCADE"), primary_key=True),
                                db.Column('document_type_id', db.Integer, db.ForeignKey(
                                    'document_type.id', ondelete="RESTRICT"), primary_key=True)
                                )
...
class OrderType(db.Model):
    ...
    required_documents = db.relationship(
        'DocumentType', secondary=order_type_documents, back_populates='order_types')
...
class DocumentType(db.Model):
    ...
    order_types = db.relationship(
        'OrderType', secondary=order_type_documents, back_populates='required_documents')
    order_documents = db.relationship(
        'OrderDocument', back_populates='document_type', lazy=True)
...
class DocumentStatus(db.Model):
    ...
    # Relationships
    order_documents = db.relationship(
        'OrderDocument', back_populates='current_status_rel', lazy=True)
    document_history = db.relationship(
        'DocumentStatusHistory', back_populates='status', lazy=True)
...
class Company(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    registrar_id = db.Column(db.Integer, db.ForeignKey(
        'registrar.id', ondelete="RESTRICT"), nullable=False)
...
class Registrar(db.Model):
    ...
    companies = db.relationship(
        'Company', back_populates='registrar', lazy=True)
...
class OrderStatus(db.Model):
    ...
    status_history = db.relationship(
        'OrderStatusHistory', back_populates='status', lazy=True)
...
class Order(db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint('base_charges >= 0',
                        name='check_base_charges_positive'),
    ...
    order_type_id = db.Column(db.Integer, db.ForeignKey(
        'order_type.id', ondelete="RESTRICT"), nullable=False)
    company_id = db.Column(db.Integer, db.ForeignKey(
        'company.id', ondelete="RESTRICT"), nullable=False)
    status_id = db.Column(db.Integer, db.ForeignKey(
        'order_status.id', ondelete="RESTRICT"), nullable=False)
    ...
    tasks = db.relationship(
        'Task', back_populates='order', lazy=True, cascade="all, delete-orphan")

    documents = db.relationship(
        'OrderDocument', back_populates='order', lazy=True, cascade="all, delete-orphan")
    receipts = db.relationship(
        'Receipt', back_populates='order', lazy=True, cascade="all, delete-orphan")
    expenses = db.relationship(
        'Expense', back_populates='order', lazy=True, cascade="all, delete-orphan")
    comments = db.relationship(
        'Comment', back_populates='order', lazy=True, cascade="all, delete-orphan")
    settlements = db.relationship(
        'Settlement', back_populates='order', lazy=True, cascade="all, delete-orphan")
    status_history = db.relationship('OrderStatusHistory', back_populates='order',
                                     lazy=True, cascade="all, delete-orphan", order_by='OrderStatusHistory.changed_at')
    ...
    def calculate_total_receipts(self):
        """Calculate total receipts for this order using a direct query for accuracy within transactions."""
        if not self.id:
            return 0.0

        total = db.session.query(db.func.sum(Receipt.amount)).filter_by(
            order_id=self.id).scalar()
        return total or 0.0
    ...
    def validate_payment_status(self):
        """Validate and update payment status based on total receipts."""
        if not self.id:
            return
        order = db.session.query(Order).get(self.id)
        total_receipts = order.calculate_total_receipts()
        if total_receipts >= order.fees:
    ...
    def create_settlement(self):
        ...
        settlement = Settlement(
            ...
            order_id=self.id,
            partner_share=partner_amount,
            self_share=self_amount
        )
        return settlement
...
class OrderStatusHistory(db.Model):
    ...
    def __repr__(self):
        return f'<OrderStatusHistory Order:{self.order_id} Status:{self.status_id} at {self.changes_at}>'
...
class OrderDocument(db.Model):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint('order_id', 'document_type_id',
                            name='uq_order_document'),
    )

    id = db.Column(db.Integer, primary_key=True)
    order_id = db.Column(db.Integer, db.ForeignKey(
        'order.id', ondelete="CASCADE"), nullable=False)
    document_type_id = db.Column(db.Integer, db.ForeignKey(
        'document_type.id', ondelete="RESTRICT"), nullable=False)
    current_status_id = db.Column(db.Integer, db.ForeignKey(
        'document_status.id', ondelete="RESTRICT"), nullable=False)
    file_path = db.Column(db.String(255), nullable=True)
    uploaded_at = db.Column(db.DateTime, nullable=True)  # When client uploaded
    # When submitted to authorities
    submitted_at = db.Column(db.DateTime, nullable=True)

    # Relationships
    order = db.relationship('Order', back_populates='documents')
    document_type = db.relationship(
        'DocumentType', back_populates='order_documents')
    current_status_rel = db.relationship(
        'DocumentStatus', back_populates='order_documents')
    status_history = db.relationship('DocumentStatusHistory', back_populates='document',
                                     lazy=True, cascade="all, delete-orphan", order_by='DocumentStatusHistory.date_created')
...
class DocumentStatusHistory(db.Model):
    ...
    document_id = db.Column(db.Integer, db.ForeignKey(
        'order_document.id', ondelete="CASCADE"), nullable=False)
    status_id = db.Column(db.Integer, db.ForeignKey(
        'document_status.id', ondelete="RESTRICT"), nullable=False)
    date_created = db.Column(
        db.DateTime, default=datetime.utcnow, nullable=False)
    # e.g., "Photo unclear, resubmission required"
    notes = db.Column(db.Text, nullable=True)
    # User ID who made the change
    changed_by = db.Column(db.Integer, nullable=True)
    ...
    document = db.relationship(
        'OrderDocument', back_populates='status_history')
    status = db.relationship(
        'DocumentStatus', back_populates='document_history')
...
class Task(db.Model):
    ...
    def __repr__(self):
        return f'<Task {self.title} for Order {self.order_id}>'
...
class Settlement(db.Model):
    ...
    __table_args__ = (
        ...
        Index('idx_settlement_date', 'date'),
    ...
    self_share = db.Column(db.Float, nullable=False)
    date = db.Column(db.DateTime, default=datetime.utcnow)
...
def update_document_timestamps(mapper, connection, target):
    ...
    if status_history.has_changes():
        ...
        new_status = db.session.query(
            DocumentStatus).get(target.current_status_id)
